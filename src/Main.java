public class Main {
    public static void main(String[] args) {
        System.out.println("Kycklinggryta med curry");
        System.out.println("Ingredienser:");
        System.out.println("700 g kycklingfilé\n" +
                "2 röda paprikor\n" +
                "2 gula lökar\n" +
                "2 msk smör, att steka i\n" +
                "ca 3 msk curry\n" +
                "1 msk paprikapulver\n" +
                "5 dl vispgrädde\n" +
                "2 dl crème fraîche\n" +
                "2 msk koncentrerad apelsinjuice\n" +
                "2 msk kinesisk soja\n" +
                "2 krm svartpeppar");
        System.out.println("Instruktion:\n" +
                "1. Skär kycklingfiléerna i bitar.\n" +
                "2. Skölj, kärna ur och skär paprikan i bitar.\n" +
                "3. Skala och skär löken i klyftor.\n" +
                "4. Bryn kycklingen i smör i en stor gryta.\n" +
                "5. Tillsätt paprika, lök, curry och paprikapulver och låt fräsa cirka 3\n" +
                "minuter.\n" +
                "6. Tillsätt vispgrädde, crème fraiche, apelsinjuice, soja och peppar. Låt\n" +
                "bli varmt och se till att kycklingen är genomstekt. Servera!");

        System.out.println("Bolognese med Tagliatelle");
        System.out.println("\nIngredienser:"
        +"\n400 g nötfärs\n" +
                " 4 msk Zeta extra jungfruolivolja classico\n" +
                " 1 gul lök, finhackad\n" +
                " 2 klyftor vitlök, finhackade\n" +
                " 1 morot, finhackad\n" +
                " 1 stjälk stjälkselleri, finhackad\n" +
                " 1 dl Zeta Matlagningsvin Vitt\n" +
                " 1 förp krossade tomater\n" +
                " 2 msk Zeta Tomatpuré\n" +
                " 1 köttbuljongtärning\n" +
                " 2 lagerblad\n" +
                " 1 tsk torkad timjan\n" +
                " 1 tsk torkad oregano\n" +
                " salt\n" +
                " nymalen svartpeppar\n" +
                " 1 förp Zeta Tagliatelle Casa Di Luca\n" +
                " Zeta Grana Padano 16 mån, nyriven");

        System.out.println("\nInstruktioner:"
                +" Instruktion:\n" +
                " 1.Stek nötfärs på hög värme i olivolja i en gryta tills färsen är\n" +
                " genomstekt.\n" +
                " 2.Tillsätt lök, vitlök, morot och selleri.\n" +
                " 3.Stek ytterligare 1 minut.\n" +
                " 4.Lägg i tomatpuré, vitt vin och krossade tomater.\n" +
                " 5.Låt koka upp och lägg i buljongtärning och örter, låt puttra ca 45 minuter under lock.\n" +
                " 6.Rör regelbundet och späd eventuellt med lite vatten.\n" +
                " 7.Smaka av med salt och peppar och servera med nykokt pasta och\n" +
                " Grana Padano");

    }
}